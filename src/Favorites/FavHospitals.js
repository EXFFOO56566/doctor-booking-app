...
class FavHospitals extends Component {
  ...
  deleteAllFavhospitalsData = async () => {
    this.setState({
      isLoading: true,
    });
    const id = await AsyncStorage.getItem("projectProfileId");
    axios
      .post(CONSTANT.BaseUrl + "user/delete_wishlists", {
        profile_id: id,
        type: "hospitals",
      })
      .then(async (response) => {
  ...
  render() {
    ...
    return (
      <View style={styles.favHospitalsContainer}>
        ...
        <TouchableOpacity
        onPress={()=> this.deleteAllFavhospitalsData()}
        style={{height:45,flexDirection:"row",marginHorizontal:10,alignItems:"center",borderColor:"#3D4461",borderWidth:1,justifyContent:"space-between",borderRadius:8,marginBottom:8,marginTop:15,paddingHorizontal:10}}>
          <Text style={styles.DocName}>{CONSTANT.FavoritesRemovedAllHospitals}</Text>
