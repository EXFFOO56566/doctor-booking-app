...
class MessagesMain extends Component {
  ...
  fetchMessages = async () => {
    ...
    const response = await fetch(CONSTANT.BaseUrl + 'chat/list_users/?user_id=' + Pid);
    ...
    if (
      ...
      this.setState({fetchMessageList: json,fetchMessageListNew: json, isLoading: false});
    }
  };
   handleSearch = text => {
    let formattedQuery = text.toLowerCase();
    let searchData = this.state.fetchMessageListNew.filter(item => {
      return item.user_name.toLowerCase().includes(formattedQuery);
    });
    this.setState({fetchMessageList:searchData})
      ...
      <View style={styles.container}>
        ...
        {isLoading && (
         ...
         <TextInput
            ...
            placeholder={CONSTANT.MessageSearchUsers}
            onChangeText={title => this.handleSearch(title)}
            style={styles.TextInputSearchInboxStyle}
          />
        <ScrollView>
